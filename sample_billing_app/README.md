...
Each generated project includes a CLI tool named `sample_billing_app`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run sample_billing_app server run --reload     # Start dev server
uv run sample_billing_app server routes           # Show all routes
# Database commands
uv run sample_billing_app db migrate -m "message" # Create migration
uv run sample_billing_app db upgrade              # Apply migrations
uv run sample_billing_app db downgrade            # Rollback migration
# User commands
uv run sample_billing_app user create-admin       # Create admin user
uv run sample_billing_app user create             # Create regular user
uv run sample_billing_app user list               # List all users
```
...
# Server
uv run sample_billing_app server run --reload
uv run sample_billing_app server routes

# Database (Alembic wrapper)
uv run sample_billing_app db init
uv run sample_billing_app db migrate -m "Add users"
uv run sample_billing_app db upgrade

# Users
uv run sample_billing_app user create-admin       # Create admin (interactive)
uv run sample_billing_app user create             # Create user (interactive)
uv run sample_billing_app user list               # List all users
```
...
```bash
uv run sample_billing_app cmd seed --count 100
uv run sample_billing_app cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend sample_billing_app server run --reload` |
| Start prod server | `uv run --directory backend sample_billing_app server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend sample_billing_app server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend sample_billing_app db migrate -m "message"` |
| Apply migrations | `uv run --directory backend sample_billing_app db upgrade` |
| Rollback migration | `uv run --directory backend sample_billing_app db downgrade` |
| Show current | `uv run --directory backend sample_billing_app db current` |
| Show history | `uv run --directory backend sample_billing_app db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend sample_billing_app user create-admin` |
| Create user | `uv run --directory backend sample_billing_app user create` |
| List users | `uv run --directory backend sample_billing_app user list` |

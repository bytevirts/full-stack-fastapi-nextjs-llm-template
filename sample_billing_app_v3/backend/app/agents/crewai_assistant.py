    ...
    def _register_handlers(self):
        ...
        def on_crew_completed(source, event: CrewKickoffCompletedEvent):
            ...
            self._event_queue.put(
                {
                    "type": "crew_complete",
                    "result": str(output.raw if hasattr(output, "raw") else output)
                    if output
                    else "",
                }
            )
            ...
            self._event_queue.put(
                {
                    "type": "task_completed",
                    "task_id": str(getattr(task, "id", "")) if task else "",
                    "output": str(output.raw if hasattr(output, "raw") else output)
                    if output
                    else "",
                    "agent": getattr(getattr(task, "agent", None), "role", "Unknown")
                    if task
...
class CrewAIAssistant:
    ...
    def _build_crew(self) -> Crew:
        ...
        process = (
            Process.hierarchical if self.config.process == "hierarchical" else Process.sequential
        )
                ...
                task_results.append(
                    {
                        ...
                        "output": str(
                            task.output.raw if hasattr(task.output, "raw") else task.output
                        ),
                    }
                )

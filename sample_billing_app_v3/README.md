...
Each generated project includes a CLI tool named `sample_billing_app_v3`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run sample_billing_app_v3 server run --reload     # Start dev server
uv run sample_billing_app_v3 server routes           # Show all routes
# Database commands
uv run sample_billing_app_v3 db migrate -m "message" # Create migration
uv run sample_billing_app_v3 db upgrade              # Apply migrations
uv run sample_billing_app_v3 db downgrade            # Rollback migration
# User commands
uv run sample_billing_app_v3 user create-admin       # Create admin user
uv run sample_billing_app_v3 user create             # Create regular user
uv run sample_billing_app_v3 user list               # List all users
```
...
```yaml
scrape_configs:
  - job_name: 'sample_billing_app_v3'
    static_configs:
...
# Server
uv run sample_billing_app_v3 server run --reload
uv run sample_billing_app_v3 server routes

# Database (Alembic wrapper)
uv run sample_billing_app_v3 db init
uv run sample_billing_app_v3 db migrate -m "Add users"
uv run sample_billing_app_v3 db upgrade

# Users
uv run sample_billing_app_v3 user create-admin       # Create admin (interactive)
uv run sample_billing_app_v3 user create             # Create user (interactive)
uv run sample_billing_app_v3 user list               # List all users
```
...
```bash
uv run sample_billing_app_v3 cmd seed --count 100
uv run sample_billing_app_v3 cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend sample_billing_app_v3 server run --reload` |
| Start prod server | `uv run --directory backend sample_billing_app_v3 server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend sample_billing_app_v3 server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend sample_billing_app_v3 db migrate -m "message"` |
| Apply migrations | `uv run --directory backend sample_billing_app_v3 db upgrade` |
| Rollback migration | `uv run --directory backend sample_billing_app_v3 db downgrade` |
| Show current | `uv run --directory backend sample_billing_app_v3 db current` |
| Show history | `uv run --directory backend sample_billing_app_v3 db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend sample_billing_app_v3 user create-admin` |
| Create user | `uv run --directory backend sample_billing_app_v3 user create` |
| List users | `uv run --directory backend sample_billing_app_v3 user list` |

### Taskiq

| Task | Command |
|------|---------|
| Start worker | `uv run --directory backend sample_billing_app_v3 taskiq worker` |
| Start scheduler | `uv run --directory backend sample_billing_app_v3 taskiq scheduler` |
